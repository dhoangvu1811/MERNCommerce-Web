...
import { ORDER_STATUS, PAYMENT_STATUS, isOrderCancellable } from '~/utils/orderConstants'
...
function OrderActions({
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
      ...
      {/* Hủy đơn hàng - khi có thể hủy (PENDING) */}
      {isOrderCancellable(order.status) && onCancelOrder && (
        <Button
          variant='outlined'
          color='error'
          startIcon={<Cancel />}
      ...
      {[ORDER_STATUS.CANCELLED, ORDER_STATUS.COMPLETED].includes(
        order.status
      ) &&
        onReorder && (
          <Button
